...
SDCond ..       sum((i,j), x(i)*VarCov(i,j)*x(j)) =l= bench_sd_eq**2;


MODEL MaxRet_Opg7 /diff_pos, diff_neg, CostDef, BudgetDef, ReturnDef, SDCond/;
...
*   *********** DEFINE WHAT IS NEEDED FOR MODEL *********
    ...
    bench_sd_eq = sd_bench(loop_no);
    
    avg_ret(i) = 1/card(q) * sum(q, Return_4(i,q));
    
    loop(i,
        loop(j,
            VarCov(i,j) = sum(q, (Return_4(i,q) - avg_ret(i)) * (Return_4(j,q) - avg_ret(j))) / (card(q)-1);
        );
    );  
    ...
    display Return_4, avg_ret, min_ret, max_ret, VarCov;
...
*   InitPortvalue is previous portfolio times actual return of the period
    ...
    solve MaxRet_Opg7 maximizing Exp_Return using nlp;
...
*   New initial portfolio values for next period
    ...
    display Port_value, Port_value_avg, Port_value_min, Port_value_max, Exp_Return.l, avg_ret, bench_sd_eq;
)

$exit

*--- Export to CSV ---
FILE ReturnFile /"PortValues_Opg7_maxR.csv"/;
...
* Write header
PUT "Loop number", "Actual portfolio","Value_avg","Value_min","Value_max"/;

* Loop over dates
LOOP(loop_no,
    PUT loop_no.tl:12:0, Port_value(loop_no):12:6, Port_value_avg(loop_no):12:6, Port_value_min(loop_no):12:6, Port_value_max(loop_no):12:6;
    PUT /;
);
...
execute_unload 'Weight_Opg7_maxR.gdx', x_across_loops;
