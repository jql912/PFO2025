...
*   Save portfolio valuee (both current and previous after costs)
    ...
    Port_value_avg(loop_next)$(ord(loop_next) = cur_loop_no + 1) = sum(i, x_start(i,loop_no)$(ord(loop_no) = cur_loop_no)*(1+avg_ret(i)));
    
    Port_value_min(loop_next)$(ord(loop_next) = cur_loop_no + 1) = sum(i, x_start(i,loop_no)$(ord(loop_no) = cur_loop_no)*(1+min_ret(i)));
    
    Port_value_max(loop_next)$(ord(loop_next) = cur_loop_no + 1) = sum(i, x_start(i,loop_no)$(ord(loop_no) = cur_loop_no)*(1+max_ret(i)));
...
*--- Export to CSV ---
FILE ReturnFile /"PortValues_Opg7.csv"/;
...
LOOP(loop_no,
    ...
    LOOP(i,
        PUT x_across_loops(i,loop_no):6:5;
    );
